...
export class AuthController {
  ...
  async login(
  ...
  ): Promise<IResponseHandler<UserTokensDto>> {
    ...
    if (!responseObject) {
      throw new HttpException(
        await this.i18n.translate('auth.cannotSignIn', { lang }),
  ...
  @Post('/register')
  async register(
    @Body() postCreateUserDto: PostCreateUserDto,
    @I18nLang() lang: string,
    ...
    if (responseObject) {
      return ResponseHandler<string>(
        HttpStatus.CREATED,
        await this.i18n.translate('auth.accountHasBeenCreated', { lang }),
      );
    }
  }
  ...
  async logout(
    ...
    await this.authService.removeRefreshTokenByToken(
      req.body.refresh_token,
      req.user.id,
    );
    return ResponseHandler<string>(
      HttpStatus.OK,
      await this.i18n.translate('auth.loggedOut', { lang }),
    );
  }
  ...
  async removeRefreshToken(
    ...
    return ResponseHandler<string>(
      HttpStatus.OK,
      await this.i18n.translate('auth.sessionRemoved', { lang }),
    );
  }
  ...
  async refreshAccessToken(
    ...
    const responseObject =
      await this.authService.generateAccessTokenFromRefreshToken(req);
    return ResponseHandler<UserTokensDto>(HttpStatus.OK, {
      access_token: responseObject.access_token,
      refresh_token: responseObject.refresh_token,
    });
  }
  ...
  @UseGuards(AccountTypeGuard())
  @Get('/authorize')
  async isUserAuthorizated(
    @Req() req: any,
    @I18nLang() lang: string,
  ): Promise<IResponseHandler<IAuthorizatedUser>> {
    if (!req.user) {
      throw new HttpException(
        await this.i18n.translate('auth.signInToContinue', { lang }),
        HttpStatus.UNAUTHORIZED,
      );
    }
